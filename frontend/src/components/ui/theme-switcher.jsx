...
export function ThemeSwitcher() {
  ...
  // Colors adapt based on the VISUAL mode, not the selected button
  const activeBg = dark ? "#333" : "#e8e8e8";
  const activeIcon = dark ? "#fff" : "#000";
  const inactiveIcon = dark ? "#777" : "#999";
  const borderColor = dark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.12)";

  const btn = (value) => ({
    className: "flex items-center justify-center w-8 h-8 rounded-[999px] cursor-pointer",
    style: theme === value ? { backgroundColor: activeBg } : {},
    onClick: () => choose(value),
    type: "button",
  });

  const icon = (value) => ({
    fill: theme === value ? activeIcon : inactiveIcon,
  });

  return (
    <div
      className="flex items-center h-8 rounded-[999px] border"
      style={{ borderColor }}
    >
      <button aria-label="System theme" {...btn("system")}>
      ...
      <button aria-label="Light theme" {...btn("light")}>
        <svg height="14" viewBox="0 0 16 16" width="14" fill="none">
          <path fillRule="evenodd" clipRule="evenodd"
      ...
      <button aria-label="Dark theme" {...btn("dark")}>
        <svg height="14" viewBox="0 0 16 16" width="14" fill="none">
          <path fillRule="evenodd" clipRule="evenodd"
